...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!checkRateLimit(ipRateLimit, ip) || !checkRateLimit(emailRateLimit, email)) {
      ...
      return NextResponse.json(
        {
          error: 'Překročili jste limit odeslání. Zkuste to prosím později.',
    ...
    if (!isHuman) {
      return NextResponse.json(
        {
          error: 'Ověření CAPTCHA selhalo. Zkuste to prosím znovu.',
    ...
    if (!SMTP2GO_API_KEY || !SMTP2GO_FROM_EMAIL || !BUSINESS_EMAIL) {
      ...
      return NextResponse.json(
        {
          error: 'Chyba konfigurace serveru. Kontaktujte prosím administrátora.',
  ...
  <div class="container">
    ...
    <div class="form-container">
      ...
      <div class="footer">
        ...
        ${new Date().toLocaleString('cs-CZ', { timeZone: 'Europe/Prague' })}</p>
      </div>
    </div>
  </div>
</body>
    ...
    return NextResponse.json(
      {
        error: 'Při zpracování vaší zprávy došlo k chybě.',
