    ...
    <div id="cookie-banner" className="cookie-banner fixed bottom-0 left-0 w-full bg-black/95 backdrop-blur-md shadow-lg py-4 z-[999] transition-transform duration-500 ease-in-out transform translate-y-0 text-white border-t border-white/10">
      <div className="container flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="cookie-content flex-1 text-center md:text-left">
          <h4 className="text-xl font-bold mb-2">🍪 Cookies na našem webu</h4>
          <p className="text-gray-light text-sm">Používáme cookies pro zlepšení uživatelského zážitku a pro analýzu návštěvnosti. Kliknutím na „Souhlasím“ vyjadřujete souhlas s použitím všech cookies.</p>
        </div>
        <div className="cookie-buttons flex flex-col md:flex-row gap-3">
          <button onClick={acceptAllCookies} className="cookie-btn btn btn-primary !bg-accent-teal !text-white text-sm px-6 py-2 rounded-full">Souhlasím</button>
          <button onClick={declineAllCookies} className="cookie-btn btn btn-secondary !bg-gray-medium !text-white text-sm px-6 py-2 rounded-full">Odmítnout</button>
          <button onClick={() => setShowSettings(true)} className="cookie-btn btn btn-secondary !bg-gray-medium !text-white text-sm px-6 py-2 rounded-full">Nastavení</button>
        </div>
      </div>
    </div>
  );
